            ...
            else
            {
                ...
                while (!this.IsAtTagStart && !this.IsAtEndOfStream && !this.IsAtDirectiveStart)
                {
                    ...
                    else
                    {
                        if (this.NextCharacter <= ' ')
                        {
                            // @EDITED sstrebl: attempt at fixing the whitespace bug (where an element containing a single whitespace is converted to an empty element)
                            _nextToken.Append(' ');
                            _ignoreNextWhitespace = false;
                            // @ENDEDIT uncomment the following lines to reestablish the initial behaviour
